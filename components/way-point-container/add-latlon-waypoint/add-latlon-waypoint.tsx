  ...
  const addLatLon = () => {
    if (!Number.isNaN(Number(tempLat)) && !Number.isNaN(Number(tempLon))) {
      props.setWaypoints([
        ...props.waypoints,
        {
          lat: parseFloat(tempLat as string),
          lng: parseFloat(tempLon as string),
          id: props.waypoints.length,
          type: tempType,
          name: `${getInitial(tempType)}${props.waypoints.filter((v) => v.type == tempType).length + 1}`,
        },
      ]);
      settempLat("");
      settempLon("");
  ...
  return (
    <Card className="p-2">
      ...
      <div className="flex items-items gap-1">
        <DropdownMenu>
          <DropdownMenuTrigger
            className={`text-xs ${getColor(tempType)} w-[40px] mt-1`}
          >
            {getInitial(tempType)}
          </DropdownMenuTrigger>
          <DropdownMenuContent onChange={(e) => console.log(e)}>
            ...
            <DropdownMenuRadioGroup>
              <DropdownMenuRadioItem
                value={WayPointType.GNSS.toString()}
                onClick={() => settempType(WayPointType.GNSS)}
              >
                GNSS
              </DropdownMenuRadioItem>
              <DropdownMenuRadioItem
                value={WayPointType.ARUCO.toString()}
                onClick={() => settempType(WayPointType.ARUCO)}
              >
                Aruco
              </DropdownMenuRadioItem>
              <DropdownMenuRadioItem
                value={WayPointType.MALLETE.toString()}
                onClick={() => settempType(WayPointType.MALLETE)}
              >
                Mallete
              </DropdownMenuRadioItem>
              <DropdownMenuRadioItem
                value={WayPointType.BOTTLE.toString()}
                onClick={() => settempType(WayPointType.BOTTLE)}
              >
                Bottle
              </DropdownMenuRadioItem>
            </DropdownMenuRadioGroup>
        ...
        <Button
          ...
          className="text-xs flex-1 mt-1 w-full text-white"
          size="sm"
        >
          Add
        </Button>
