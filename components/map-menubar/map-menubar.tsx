...
const MapMenubar = (props: MapMenubarProps) => {
  ...
  return (
    <Card
      className={`w-[65%] bg-primary/20 h-[7vh] flex items-center justify-between px-3`}
    >
      <div className="flex items-center gap-2">
        <p className="text-sm">Options</p>
        <Button
          size="icon"
          onClick={() => changeMode(WayPointType.ARUCO)}
          variant={
            props.wpType == WayPointType.ARUCO && props.mapActive
              ? "default"
              : "outline"
          }
        >
          <img src="/marker/aruco.png" className="h-[20px] w-[20px]" />
        </Button>
        <Button
          size="icon"
          onClick={() => changeMode(WayPointType.BOTTLE)}
          variant={
            props.wpType == WayPointType.BOTTLE && props.mapActive
              ? "default"
              : "outline"
          }
        >
        ...
        <Button
          size="icon"
          onClick={() => changeMode(WayPointType.GNSS)}
          variant={
            props.wpType == WayPointType.GNSS && props.mapActive
              ? "default"
              : "outline"
          }
        >
        ...
        <Button
          size="icon"
          onClick={() => changeMode(WayPointType.MALLETE)}
          variant={
            props.wpType == WayPointType.MALLETE && props.mapActive
              ? "default"
              : "outline"
          }
        >
