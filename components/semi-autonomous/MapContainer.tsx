...
const MapContainer = () => {
  ...
  const rectangleBounds = [
    [
      Number(roverPosition.lat) - sizeInDegrees.lat,
      Number(roverPosition.lng) - sizeInDegrees.lng,
    ],
    [
      Number(roverPosition.lat) + sizeInDegrees.lat,
      Number(roverPosition.lng) + sizeInDegrees.lng,
  ...
  useEffect(() => {
    ...
    const wpPaths: WayPoint[] = Array.isArray(path)
      ? path.map((pose, index) => ({
  ...
  const startIcon = L.divIcon({
    ...
    ${renderToString(
      <div className="relative">
        <div
          className={`h-[40px] w-[2px] ${getColor(WayPointType.GNSS)}`}
        ></div>
        <div
          className={`h-[20px] absolute top-0 left-0 w-[40px] ${getColor(WayPointType.GNSS)} text-xs flex items-center justify-center text-white`}
        >
          START
        </div>
      </div>,
  ...
  const waypointIcon = L.divIcon({
    html: `<div class="flex flex-col items-center">
    ${renderToString(<div className="relative"></div>)}
    </div>`,
    className: `w-20 h-20 `,
    iconSize: [40, 40],
    iconAnchor: [12, 41],
  ...
  return (
    <div className="relative">
      ...
      <div className="w-[40px] h-[40px] flex items-center justify-center text-2xl top-[90vh] left-[50%] font-bold absolute z-50 bg-sky-500/50">
        S
      </div>
...
{deliveryCoord&&<>
  ...
  />
        ...
        <div>
          ...
          <Marker
            icon={startIcon}
            position={[Number(start.lat), Number(start.lng)]}
            autoPanOnFocus={true}
          ></Marker>
        </div>
        ...
        <div>
          ...
          <Marker
            icon={endIcon}
            position={[Number(end.lat), Number(end.lng)]}
            autoPanOnFocus={true}
          ></Marker>
        </div>
