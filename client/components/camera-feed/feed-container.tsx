...
const FeedContainer = (props: FeedContainerProps) => {
  ...
  return (
    <div
      className={`bg-primary/20 rounded-md p-1 mt-5 relative overflow-hidden ${angle == 270 || angle == 90 ? "h-[500px]" : "h-[400px]"}`}
    >
      <div className="flex h-[27px] gap-2">
        <div className="p-1.5 bg-primary/30 rounded-md flex items-center">
          <div
            className={`w-[10px] h-[10px] ${statusColors[status]} rounded-full`}
          ></div>
        ...
        <Button
          ...
          variant="destructive"
          className="h-[27px]"
          onClick={() => props.setUrl((p) => p.filter((v) => v != props.url))}
        >
          Delete
        </Button>
      </div>
